...
TCPConn::TCPConn(EventLoop* l,
    ...
    XTRACE("TCPConn::[%s], fd=%d, addr=%s", name_.c_str(), sockfd, AddrToString().c_str());
}

TCPConn::~TCPConn() {
    XTRACE("name=%s, fd=%d, type=%d, status=%s, addr=%s", name().c_str(), fd_, int(type()),
            StatusToString().c_str(), AddrToString().c_str());
...
void TCPConn::Close() {
    XTRACE("fd=%d, status=%s, addr=%s", fd_, StatusToString().c_str(), AddrToString().c_str());
...
void TCPConn::SendInLoop(const void* data, size_t len) {
    ...
    if (status_ == kDisconnected) {
        XWARN("disconnected, give up writing");
        return;
    }
    ...
    if (!chan_->IsWritable() && output_buffer_.length() == 0) {
        ...
        if (nwritten >= 0) {
            ...
            if (!EVUTIL_ERR_RW_RETRIABLE(serrno)) {
                XERROR("SendInLoop write failed errno=%d,%s", serrno, strerror(serrno).c_str());
...
void TCPConn::HandleRead() {
    ...
    if (n > 0) {
        ...
        if (type() == kOutgoing) {
            ...
            XTRACE("fd=%d. We read 0 bytes and close the socket", fd_);
            status_ = kDisconnecting;
            HandleClose();
        } else {
            ...
            if (close_delay_.IsZero()) {
                XTRACE("channel (fd=%d)  DisableReadEvent. delay time %f, s. We close this connection immediately",
...
void TCPConn::HandleWrite() {
    ...
    if (n > 0) {
        ...
        if (EVUTIL_ERR_RW_RETRIABLE(serrno)) {
            XWARN("this=%p, TCPConn::HandleWrite errno=%d,%s", this, serrno, strerror(serrno).c_str());
        } else {
            HandleError();
        }
    }
