...
void DNSResolver::SyncDNSResolve() {
    ...
    if (err != 0) {
        XERROR("this=%p, getaddrinfo failed. err=%d,%s", this, err, gai_strerror(err));
    } else {
...
void DNSResolver::OnResolved(int errcode, struct addrinfo* addr) {
    if (errcode != 0) {
        if (errcode != EVUTIL_EAI_CANCEL) {
            ClearTimer();
            XERROR("DNS resolve failed, error code:%d, error msg:%s", errcode, evutil_gai_strerror(errcode));
        } else {
            XWARN("DNS resolve cancel, may be timeout");
        }

        XWARN("delete DNS base. errcode=%d, %s", errcode, strerror(errcode).c_str());
    ...
    if (addr == nullptr) {
        XERROR("this=%p, dns resolve error, addr can not be nullptr");
        evdns_base_free(dnsbase_, 0);
        dnsbase_ = nullptr;
        ClearTimer();
        OnResolved();
