...
failed:
    ...
    if (retried_ < retry_number_) {
        XWARN("this=%p, http request failed :%s retried=%d max retry_time=%d . Try again.",
                this, errmsg.c_str(), retried_, retry_number_);
        Retry();
        return;
    }
...
void Request::HandleResponse(struct evhttp_request* r) {
    ...
    if (r) {
        ...
        if (!needs_retry || retried_ >= retry_number_) {
            XWARN("this=%p, response_code=%d, retried=%d max retry_time=%d ",
                  this, r->response_code, retried_, retry_number_);
    ...
    // Retry
    if (retried_ < retry_number_) {
        XWARN("this=%p, response_code==%d, retried=%d max retry_time=%d. Try again",
                this, (r ? r->response_code : 0), retried_, retry_number_);
        Retry();
        return;
    }
    ...
    if (!r) {
        ...
        while ((oslerr = bufferevent_get_openssl_error(conn_->bufferevent()))) {
            ...
            XERROR("Openssl error: %s", buffer);
            printed_some_error = true;
        }
        if (!printed_some_error) {
//            LOG_ERROR << "socket error(" << errcode << "): "
//                << evutil_socket_error_to_string(errcode);
            XERROR("socket erro %d,%s", errcode, evutil_socket_error_to_string(errcode));
        }
    }
#endif
