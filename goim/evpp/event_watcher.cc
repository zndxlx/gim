...
bool EventWatcher::Watch(Duration timeout) {
    ...
    if (attached_) {
        ...
        if (EventDel(event_) != 0) {
            XERROR("event_del failed. fd=%d event_=%p", this->event_->ev_fd, event_);
    ...
    if (EventAdd(event_, timeoutval) != 0) {
        XERROR("event_add failed. fd=%d event_=%p", this->event_->ev_fd, event_);
...
bool PipeEventWatcher::DoInit() {
    ...
    if (evutil_socketpair(AF_UNIX, SOCK_STREAM, 0, pipe_) < 0) {
        int err = errno;
        XERROR("create socketpair ERROR errno=%d,%s", err,  strerror(err).c_str());
        goto failed;
    }
...
void PipeEventWatcher::HandlerFn(evpp_socket_t fd, short /*which*/, void* v) {
    XINFO("PipeEventWatcher::HandlerFn fd=%d, v=%p", fd, v);
