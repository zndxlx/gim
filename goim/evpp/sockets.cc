...
evpp_socket_t CreateNonblockingSocket() {
    ...
    if (fd == -1) {
        serrno = errno;
        XERROR("socket error:%s", strerror(serrno).c_str() );
    ...
    if (fcntl(fd, F_SETFD, 1) == -1) {
        serrno = errno;
        XERROR("fcntl(F_SETFD) %s", strerror(serrno));
        goto out;
    }
#endif
...
evpp_socket_t CreateUDPServer(int port) {
    ...
    if (::bind(fd, (struct sockaddr*)&local, sizeof(struct sockaddr))) {
        int serrno = errno;
        XERROR("socket bind error=d, errno=%d,%s", serrno, strerror(serrno).c_str());
        return INVALID_SOCKET;
    }
...
bool ParseFromIPPort(const char* address, struct sockaddr_storage& ss) {
    ...
    if (rc == 0) {
        XINFO("ParseFromIPPort evutil_inet_pton (AF_INET %s ...) rc=0. %s is not a valid IP address. Maybe it is a hostname.",
              host.data(), host.data());
        return false;
    } else if (rc < 0) {
        int serrno = errno;
        if (serrno == 0) {
            XINFO("[%s] is not a IP address. Maybe it is a hostname.", host.data());
        } else {
            XWARN("ParseFromIPPort evutil_inet_pton (AF_INET, %s,...) failed: %s",
                    host.data() , strerror(serrno).c_str());
        }
        return false;
    }
...
bool SplitHostPort(const char* address, std::string& host, int& port) {
    ...
    if (index == std::string::npos) {
        XERROR("Address specified error <%s> . Cannot find ':'", address);
        return false;
    }
    ...
    if (host[0] == '[') {
        if (*host.rbegin() != ']') {
            XERROR("Address specified error <%s> . '[' ']' is not pair.", address);
            return false;
        }
...
struct sockaddr_storage GetLocalAddr(evpp_socket_t sockfd) {
    ...
    if (::getsockname(sockfd, sockaddr_cast(&laddr), &addrlen) < 0) {
        XERROR("GetLocalAddr:%s", strerror(errno).c_str());
        memset(&laddr, 0, sizeof laddr);
    }
...
std::string ToIPPort(const struct sockaddr_storage* ss) {
    ...
    if (ss->ss_family == AF_INET) {
        ...
        XERROR("unknown socket family connected");
        return empty_string;
    }
...
void SetTimeout(evpp_socket_t fd, uint32_t timeout_ms) {
    ...
    if (ret != 0) {
        int err = errno;
        XERROR("setsockopt SO_RCVTIMEO ERROR %d:%s", strerror(err).c_str());
    }
}
...
void SetKeepAlive(evpp_socket_t fd, bool on) {
    ...
    if (rc != 0) {
        int serrno = errno;
        XERROR("setsockopt(SO_KEEPALIVE) failed, errno=%d, %s",serrno,  strerror(serrno).c_str());
    }
}
...
void SetReuseAddr(evpp_socket_t fd) {
    ...
    if (rc != 0) {
        int serrno = errno;
        XERROR("setsockopt(SO_REUSEADDR) failed, errno=%d, %s", serrno, strerror(serrno).c_str());
    }
}
...
void SetReusePort(evpp_socket_t fd) {
    ...
    int rc = ::setsockopt(fd, SOL_SOCKET, SO_REUSEPORT,
                          reinterpret_cast<const char*>(&optval), static_cast<socklen_t>(sizeof optval));
    XINFO("setsockopt SO_REUSEPORT fd=%d, rc=%d", fd, rc);
    if (rc != 0) {
        int serrno = errno;
        XERROR("setsockopt(SO_REUSEPORT) failed, errno=%d,%s", serrno, strerror(serrno).c_str());
    }
#endif
}
...
void SetTCPNoDelay(evpp_socket_t fd, bool on) {
    ...
    if (rc != 0) {
        int serrno = errno;
        XERROR("setsockopt(TCP_NODELAY) failed, errno=%d,%s", serrno, strerror(serrno).c_str());
    }
}
