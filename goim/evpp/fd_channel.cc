...
void FdChannel::AttachToLoop() {
    ...
    if (EventAdd(event_, nullptr) == 0) {
        ...
        XERROR("this=%p, fd=%d with event %s attach to event loop failed", this, fd_, EventsToString().c_str());
    }
}
...
void FdChannel::DetachFromLoop() {
    ...
    if (EventDel(event_) == 0) {
        ...
        XERROR("DetachFromLoop this=%p, fd=%d  with event %s  detach from event loop failed",
                this, fd_, EventsToString().c_str());
    }
}
