...
bool Service::Init(const ConnectionCallback& cb) {
    ...
    if (!tcp_srv_->Init()) {
        ...
        delete tcp_srv_;
        is_stopped_ = true;
        XWARN("tcpserver on %s init failed", listen_addr_.c_str());
        return false;
    }
    XINFO("http server init success");
    return true;
}

bool Service::Start() {
    if (is_stopped_) {
        XWARN("init failed, so not to start");
        return false;
    }
    ...
    if (!tcp_srv_->Start()) {
        XWARN("tcpserver on %s  start faile", listen_addr_.c_str());
        return false;
    }
    XINFO("http server start on %s suc", listen_addr_.c_str());
    return true;
}
