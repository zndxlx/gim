    ...
    ~RecvThread() {
        ...
        if (this->thread_ && this->thread_->joinable()) {
            try {
                ...
                XERROR("Caught a system_error:%s", e.what());
            }
        }
    }
...
bool Server::Init(const std::string& listen_ports/*like "53,5353,1053"*/) {
    ...
    for (auto& s : vec) {
        ...
        if (i <= 0) {
            XERROR("Cannot convert ["%s"] to a integer. 'listen_ports' format wrong.", s.c_str());
            return false;
        }
        v.push_back(i);
    }
...
bool Server::Start() {
    if (!message_handler_) {
        XERROR("MessageHandler DO NOT set!");
        return false;
    }
...
void Server::RecvingLoop(RecvThread* thread) {
    XINFO("UDPServer is running at 0.0.0.0:%d", thread->port());
    ...
    while (true) {
        ...
        if (readn >= 0) {
            ...
            XERROR("errno=%d,%s", eno, strerror(eno).c_str());
        }
    }

    XINFO("fd=%d, port=%d, UDP server existed.", thread->fd(), thread->port());
