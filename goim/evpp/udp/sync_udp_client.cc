...
bool Client::Connect() {
    ...
    if (ret != 0) {
        XERROR("Failed to connect to remote, %s, errno=%d, %s", sock::ToIPPort(&remote_addr_).c_str(), errno,
...
std::string Client::DoRequest(const std::string& data, uint32_t timeout_ms) {
    if (!Send(data)) {
        int eno = errno;
        XERROR("sent failed, errno=%d, %s, dlen=%d", eno, strerror(eno).c_str(), data.size());
        return "";
    }
    ...
    if (readn >= 0) {
        ...
        XERROR("errno=%d,%s ,recvfrom return -1", err, strerror(err).c_str());
    }
